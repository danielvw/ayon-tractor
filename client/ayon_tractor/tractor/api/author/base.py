'''Tractor Job Authoring API - public functions'''
from __future__ import print_function
import datetime
import json
import tractor.base.EngineClient as EngineClient
import tractor.base.alf2json as alf2json

from . import RequiredValueError, ParentExistsError, SpoolError
...
class SubtaskMixin(object):
    ...
    def spool(self, block=False, owner=None, spoolfile=None, spoolhost=None,
              hostname=None, port=None, asJSON=True):
        ...
        # send spool message
        try:
            jtxt = self.asJSON() if asJSON else self.asTcl()

            result = ModuleEngineClient.spool(
                jtxt, skipLogin=True, block=block, owner=owner,
...
class Job(KeyValueElement, SubtaskMixin, CleanupMixin, PostscriptMixin):
    ...
    def asJSON(self):
        alftcl = self.asTcl()
        return alf2json.ParseAlfJobAsString(alftcl)
...
_testJobText = '''
Job -title {job authoring test} -subtasks {
  Task {a task} -cmds {
    RemoteCmd {{echo} {API hello}} -service {PixarRender}
  }
}
'''
...
def testBasicValidation():
    print('==---------==\nTest: job authoring ...\n');
    job = Job(title='job authoring test');
    task = job.newTask(title='a task');
    task.newCommand(argv=['echo', 'API hello'], service='PixarRender');
    jtcl = job.asTcl()
    try:
        print("generated job:\n", jtcl)
        jjson = alf2json.ParseAlfJobAsString(jtcl)
        print("... and reparsed to json ok")
        if jtcl.strip() == _testJobText.strip():
            # jid = job.spool()  # might go here
            print("--> success  (validated job text only, not submitted)")
        else:
            print("--> FAIL, mismatch to job text reference")
    except Exception as err:
        print("--> FAIL, error parsing of constructed job\n", err)
