...
# ------------------------------------------------------------------------------
#
# Copyright (c) 2010-2023 Pixar. All rights reserved.
#
...
def hostnamePortForEngine(engineName):
    ...
    if len(parts) == 1:
        ...
        if not parts[1].isdigit():
            raise EngineClientError("'%s' must be a numeric value for port." % parts[1])
...
class EngineClient(object):
    ...
    def open(self):
        ...
        # response['tsid'] is garanteed to be valid: self.conn.Login() would have
        # raised a TrHttpError exception otherwise
    ...
    def fetchBladesAsJSON(self, filterName=None):
        ...
        attrs = {"q": "blades"}
        if filterName:
            attrs["filter"] = urllib2.quote(filterName + ".bladelist")
    ...
    def addNote(self, itemtype, itemid, text):
        """Add a note/comment to the given job or blade."""
        attrs = {"q": "add", "itemtype": itemtype, "notetype": "comment",
                 "itemid": itemid, "notetext": text}
        return self._transaction(self.NOTE, attrs)
    ...
    def spool(self, jobData, hostname=None, filename=None, owner=None, format=None, skipLogin=False, block=False):
        ...
        filename = filename or "generated_by_%s" % sys.argv[0]
    ...
    def close(self):
        ...
        if not self.tsid:
            ...
            try:
                ...
                if "Tractor relogin required." != str(err):
                    # Expect relogin error if the current tsid is invalid,
                    # in which case we consider the session closed already.
                    # Otherwise something else bad happened ...
                    raise
            # Clear tsid since connection is now closed.
            # Empty tsid will cause some callers to create a new engine session.
            # tsid is also used by isOpen() method, will now report false
            self.tsid = None
